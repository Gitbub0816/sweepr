...
-- ============================================================================
-- Paste this entire file into the Neon SQL editor (or run via psql) to GUARANTEE
-- every table, column, index, constraint, type and the migration-tracking table
-- exist and are shaped correctly, regardless of which migrations were applied by
-- hand before the migration runner existed.
--
--   • Safe to run repeatedly — every statement is CREATE/ALTER ... IF NOT EXISTS
--     or wrapped in a duplicate-safe DO block. It NEVER drops data.
--   • Heals the known gaps (e.g. cleaner_availability.active, the cleaners
--     settings columns) that a CREATE TABLE IF NOT EXISTS no-op had skipped.
--   • Finally, records every numbered migration in schema_migrations so the
--     automated runner treats the database as fully up to date.
--
-- This file is intentionally NOT run by migrate.mjs (it isn't numbered); it's a
-- manual, one-shot repair / safety net.
-- ============================================================================

BEGIN;

-- Migration bookkeeping table (the runner uses this to skip applied migrations).
...
-- ----------------------------------------------------------------------------
-- Consolidated, idempotent structural schema (generated from all migrations)
-- ----------------------------------------------------------------------------
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- Mark every numbered migration as applied so the runner is in sync.
-- ----------------------------------------------------------------------------
...
-- Done. Re-run anytime; it will report no changes once the schema is correct.
