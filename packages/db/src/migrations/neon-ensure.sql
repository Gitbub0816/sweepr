...
-- Not auto-run by the migration runner (it isn't numbered).
-- ============================================================================
...
INSERT INTO schema_migrations (filename) VALUES
  ...
  ('006_prelaunch_status.sql'),
  ('007_training_system.sql'),
  ('008_training_seed.sql'),
  ('008a_truncate_modules_lessons1-2.sql'),
  ('008b_lessons3-5.sql'),
  ('008c_lessons6-8.sql'),
  ('008d_lessons9-10_standard.sql'),
  ('008e_service_lessons.sql'),
  ('008f_quiz_mod1-6.sql'),
  ('008g_quiz_mod7-10_svc1-5.sql'),
  ('008h_quiz_svc6-8.sql'),
  ('009_admin_invites_device_tokens.sql'),
  ('010_service_areas.sql'),
  ('011_course_builder.sql'),
  ('012_day_of_service.sql'),
...
CREATE TABLE IF NOT EXISTS pricing_rules (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL, description TEXT, market_city TEXT, market_state TEXT, market_zip TEXT,
  status TEXT NOT NULL DEFAULT 'draft', currency TEXT NOT NULL DEFAULT 'USD',
  base_fee_cents INTEGER NOT NULL DEFAULT 0, minimum_booking_price_cents INTEGER NOT NULL DEFAULT 0,
  maximum_booking_price_cents INTEGER,
  sqft_pricing_enabled BOOLEAN NOT NULL DEFAULT TRUE, sqft_included_in_base INTEGER DEFAULT 750,
  sqft_tiers_json JSONB, sqft_overage_rate_cents_per_sqft INTEGER, sqft_custom_quote_threshold INTEGER,
  bedrooms_included_in_base INTEGER DEFAULT 1, price_per_extra_bedroom_cents INTEGER DEFAULT 0, bedroom_tiers_json JSONB,
  bathrooms_included_in_base NUMERIC DEFAULT 1, price_per_extra_bathroom_cents INTEGER DEFAULT 0,
  half_bathroom_price_cents INTEGER DEFAULT 0, bathroom_tiers_json JSONB,
  distance_pricing_enabled BOOLEAN NOT NULL DEFAULT FALSE, free_distance_miles NUMERIC DEFAULT 0,
  price_per_mile_cents INTEGER DEFAULT 0, distance_calculation_origin TEXT,
  maximum_service_distance_miles NUMERIC, long_distance_surcharge_cents INTEGER,
  property_type_adjustments_json JSONB, service_type_multiplier_json JSONB,
  service_type_fixed_surcharge_json JSONB, intensity_multiplier_json JSONB,
  pet_pricing_enabled BOOLEAN NOT NULL DEFAULT FALSE, pet_base_fee_cents INTEGER DEFAULT 0,
  price_per_pet_cents INTEGER DEFAULT 0, max_pet_count_before_custom_quote INTEGER,
  urgency_pricing_enabled BOOLEAN NOT NULL DEFAULT FALSE, urgency_rules_json JSONB,
  recurring_discount_enabled BOOLEAN NOT NULL DEFAULT FALSE, recurring_discount_json JSONB,
  rounding_enabled BOOLEAN NOT NULL DEFAULT TRUE, rounding_strategy TEXT DEFAULT 'end_in_9',
  rounding_increment_cents INTEGER DEFAULT 1000, rounding_ending_cents INTEGER DEFAULT 900,
  maximum_auto_quote_price_cents INTEGER, requires_custom_quote_above_cents INTEGER,
  max_discount_bps INTEGER, max_dynamic_adjustment_bps INTEGER,
  active_from TIMESTAMPTZ, active_until TIMESTAMPTZ, version INTEGER NOT NULL DEFAULT 1,
  supersedes_rule_id UUID REFERENCES pricing_rules(id), created_by TEXT NOT NULL,
  approved_proposal_id UUID, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS pricing_addons (
  ...
  addon_key TEXT NOT NULL, addon_name TEXT NOT NULL, addon_description TEXT,
  price_cents INTEGER NOT NULL DEFAULT 0, estimated_minutes INTEGER, cleaner_payout_adjustment_cents INTEGER,
  requires_customer_disclosure BOOLEAN NOT NULL DEFAULT FALSE, requires_cleaner_acknowledgement BOOLEAN NOT NULL DEFAULT FALSE,
  is_active BOOLEAN NOT NULL DEFAULT TRUE, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_pricing_addons_rule ON pricing_addons (pricing_rule_id);
CREATE TABLE IF NOT EXISTS pricing_quotes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), customer_id UUID, booking_id UUID,
  pricing_rule_id UUID NOT NULL REFERENCES pricing_rules(id), pricing_rule_version INTEGER NOT NULL,
  quote_input_json JSONB NOT NULL, quote_output_json JSONB NOT NULL,
  customer_total_cents INTEGER NOT NULL, estimated_cleaner_payout_cents INTEGER, estimated_platform_revenue_cents INTEGER,
  requires_custom_quote BOOLEAN NOT NULL DEFAULT FALSE, expires_at TIMESTAMPTZ, accepted_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS pricing_change_proposals (
  ...
  declined_at TIMESTAMPTZ, decline_reason TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
