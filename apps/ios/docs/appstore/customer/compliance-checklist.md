...
| **5.1.1(v)** Account deletion | Missing in-app deletion | SHIPPED: Account → Delete account performs the confirm-by-typing flow and calls POST /account/delete (hard delete + Clerk identity removal). |
...
| **2.1** Completeness | Broken demo / placeholder content | Provide a working demo account (`review-notes.md`). Every screen renders REAL API data with first-class loading/empty/error states — no placeholder or sample data ships in any production path. |
...
- [x] In-app account deletion ships on the Account screen (5.1.1(v)).
