...
/**
 * Shared helpers for the Sweepr Business BFF (Cloudflare Pages Functions).
 *
...
export const APP_ORIGIN = "https://business.getsweepr.com";
/** This app's broker app id, stamped into minted API tokens. */
export const APP_ID = "business";
export const SESSION_COOKIE = "__Host-sweepr_business_session";
export const LOGIN_COOKIE = "__Host-sweepr_business_login";
...
export function checkAuthConfig(env: BusinessAuthEnv): Response | null {
  if (env.CENTRAL_AUTH_ENABLED !== "true") return json(503, { error: "central_auth_disabled" });
  if (!env.BROKER_KEY_BUSINESS) return json(503, { error: "auth_unconfigured" });
  return null;
}
...
export async function brokerFetch(
  ...
  try {
    const res = await fetch(`${brokerUrl(env)}${path}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${env.BROKER_KEY_BUSINESS}`,
        "Content-Type": "application/json",
