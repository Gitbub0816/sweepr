...
export function MembersPage() {
  ...
  const inputCls =
    "w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-charcoal outline-none transition focus:border-platinum-400 focus:ring-2 focus:ring-platinum-400/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white";
            ...
            <div className="space-y-3">
              {members.map((m, i) => (
                <Card key={m.id ?? i} className="flex items-center justify-between gap-4">
                  ...
                  {m.role && (
                    <span className="shrink-0 rounded-full bg-platinum-50 px-3 py-1 text-xs font-medium capitalize text-platinum-700 dark:bg-slate-800 dark:text-platinum-400">
                      {m.role}
                    </span>
                  )}
                </Card>
