...
export function SignInPage() {
  ...
  async function handlePhoneSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const phoneFactorId = result.supportedFirstFactors?.find(
      ...
      await signIn.prepareFirstFactor({
        strategy: "phone_code",
        phoneNumberId: (phoneFactorId as { phoneNumberId: string }).phoneNumberId,
      });
      setPhoneStage("code");
    } catch (err: unknown) {
      setError(
      ...
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-8 shadow-xl dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="mb-5 flex rounded-xl border border-slate-200 p-1 dark:border-slate-700">
          {(["email", "phone"] as Method[]).map((m) => (
            <button
              key={m}
              type="button"
              onClick={() => { setMethod(m); setError(""); setPhoneStage("form"); }}
        ...
        {method === "phone" && phoneStage === "code" && (
          <form onSubmit={(e) => void handlePhoneVerify(e)} className="space-y-4">
            ...
            <button type="button" onClick={() => setPhoneStage("form")} className="w-full text-center text-sm text-slate-500 hover:text-slate-700">
              ← Change number
            </button>
          </form>
        )}
