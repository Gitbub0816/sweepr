...
// GET /insurance/me — the cleaner's own policy record.
// Sweepr does not provide coverage: a cleaner buys their own general
// liability policy (optionally via the Coverdash affiliate link the cleaner
// app shows) and uploads the certificate for review, so there are no
// enrollment prerequisites to report here.
insuranceRouter.get("/me", async (c) => {
  const user = c.get("user");
  const db = getDb(c.env.DATABASE_URL);

  const [cleaner] = await db`
    SELECT cl.id FROM cleaners cl JOIN users u ON u.id = cl.user_id
    WHERE u.clerk_id = ${user.clerkId}
  `;
  if (!cleaner) return c.json({ insurance: null });

  const [row] = await db`
    SELECT * FROM cleaner_insurance WHERE cleaner_id = ${cleaner.id}
  `;
  return c.json({ insurance: row ?? null });
});
...
insuranceAdminRouter.get("/", async (c) => {
  ...
  // Every row here is a cleaner's own policy — Sweepr does not provide
  // coverage. Legacy rows may still carry coverage_type 'sweepr_program' from
  // the withdrawn program; they no longer count as coverage anywhere.
  const rows = await db`
