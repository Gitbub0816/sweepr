...
export const APP_ORIGIN = "https://app.getsweepr.com";
/** This app's broker app id, stamped into minted API tokens. */
export const APP_ID = "customer";
export const SESSION_COOKIE = "__Host-sweepr_customer_session";
export const LOGIN_COOKIE = "__Host-sweepr_customer_login";
...
export function checkAuthConfig(env: CustomerAuthEnv): Response | null {
  if (env.CENTRAL_AUTH_ENABLED !== "true") return json(503, { error: "central_auth_disabled" });
  if (!env.BROKER_KEY_CUSTOMER) return json(503, { error: "auth_unconfigured" });
  return null;
}

// ── Broker client ────────────────────────────────────────────────────────────

export function brokerUrl(env: CustomerAuthEnv): string {
...
export async function brokerFetch(
  ...
  try {
    const res = await fetch(`${brokerUrl(env)}${path}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${env.BROKER_KEY_CUSTOMER}`,
        "Content-Type": "application/json",
